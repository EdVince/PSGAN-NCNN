...
std::vector<pair<cv::Point,cv::Point>> detect(cv::Mat image)
{
    ...
    for (int i = 0; i < face_info.size(); i++)
    {
        ...
        //faces.push_back(dlib::rectangle(face.x1, face.y1, face.x2, face.y2));
...
{
    ncnn::Net LD;
    LD.load_param("assert/slim_160_latest-opt.param");
    LD.load_model("assert/slim_160_latest-opt.bin");
    ...
    const float norm_vals[3] = { 1.f / 127.f,1.f / 127.f, 1.f / 127.f };
    in.substract_mean_normalize(mean_vals, norm_vals);

    ncnn::Extractor ex = LD.create_extractor();
    ex.input("input1", in);
    ncnn::Mat keyPoint;
